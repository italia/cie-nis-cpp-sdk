...
cie_BerReader::cie_BerReader (cie_PN532 *cie, cie::nis::Sod *sod) :
_cie(cie), sod(sod), _currentOffset(0)
{   
}
...
bool cie_BerReader::readTriples(const cie_EFPath filePath, cieBerTripleCallbackFunc callback, word *length, const byte maxDepth) {
  ...
  byte oid_SHA256[] = {0x60, 0x86, 0x48, 0x01, 0x65, 0x03, 0x04, 0x02, 0x01}; //2.16.840.1.101.3.4.2.1
  bool result = true;
  *length = 0;
  do {
    word tripleLength;
    ...
    //read the value of this triple
    if(tripleStack[currentDepth-1].contentLength > 0)
    {
	    tripleStack[currentDepth-1].value = new byte[tripleStack[currentDepth-1].contentLength]; 
	    valueCache.push_back(tripleStack[currentDepth-1].value);
	    _cie->readBinaryContent(filePath, tripleStack[currentDepth-1].value, tripleStack[currentDepth-1].contentOffset, tripleStack[currentDepth-1].contentLength);
    }
    ...
    if (*callback != NULL) {
      if (!(*callback)(&tripleStack[currentDepth-1], sod)) {
